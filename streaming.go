...
// StreamChunk represents one item yielded during streaming generation. A data
// chunk contains a partial Block that can be combined with other chunks to form
// complete blocks. A terminal error chunk sets Err and contains no payload.
//
...
// It takes a GenerationRequest and returns an iterator that yields data chunks or one
// terminal error chunk.
//
// # Usage Metadata in Streaming
//
// When a provider supplies usage metrics, a successful stream emits them as its
// final block. That StreamChunk has BlockType MetadataBlockType and contains token
// usage information in the Metadata map.
//
...
// Error handling:
//
//   - Yield a StreamChunk with Err set when validation, provider, transport, or decoding fails
//   - Error chunks must not contain payload fields
//   - Once an error chunk is yielded, no further chunks may be yielded
...
//
//	func (g *MyGenerator) Stream(ctx context.Context, request GenerationRequest) iter.Seq[StreamChunk] {
...
//	        stream := g.api.StartStream(convertRequest(request))
//	        defer stream.Close()
//
...
//	                }
//	            case "error":
//	                yield(StreamChunk{Err: event.Error})
//	                return
//	            }
//	        }
//	    }
