...
func TestStreamingAdapter_responsesThinkingSummariesSeparatedByNewlines(t *testing.T) {
	mockGen := &mockStreamingGenerator{
		chunks: []StreamChunk{
			{Block: Block{
				...
				ModalityType: Text,
				MimeType:     "text/plain",
				Content:      Str("First summary sentence."),
			...
			{Block: Block{
				...
				ModalityType: Text,
				MimeType:     "text/plain",
				Content:      Str("Second summary sentence."),
				ExtraFields: map[string]interface{}{
					ThinkingExtraFieldGeneratorKey: ThinkingGeneratorResponses,
				},
			}, CandidatesIndex: 0},
			{Block: Block{
				...
				ExtraFields: map[string]interface{}{
					ThinkingExtraFieldGeneratorKey:      ThinkingGeneratorResponses,
					ResponsesExtraFieldReasoningID:      "rs_mock_123",
					ResponsesExtraFieldEncryptedContent: "encrypted_mock_payload",
				},
			}, CandidatesIndex: 0},
			{Block: MetadataBlock(Metadata{UsageMetricInputTokens: 1, UsageMetricGenerationTokens: 2}), CandidatesIndex: 0},
		},
	}
	...
	if block.BlockType != Thinking {
		t.Fatalf("block type = %s, want %s", block.BlockType, Thinking)
	}
	if got, want := block.Content.String(), "First summary sentence.\nSecond summary sentence."; got != want {
		t.Fatalf("compressed thinking content = %q, want %q", got, want)
	}
	if got := block.ExtraFields[ResponsesExtraFieldReasoningID]; got != "rs_mock_123" {
		t.Fatalf("reasoning ID extra field = %v, want rs_mock_123", got)
	}
}

func TestStreamingAdapter_geminiThinkingSummariesSeparatedByNewlines(t *testing.T) {
	mockGen := &mockStreamingGenerator{
		chunks: []StreamChunk{
			{Block: Block{
				BlockType:    Thinking,
				ModalityType: Text,
				MimeType:     "text/plain",
				Content:      Str("Considering the constraints."),
				ExtraFields: map[string]interface{}{
			...
			{Block: Block{
				...
				ModalityType: Text,
				MimeType:     "text/plain",
				Content:      Str("Checking the final answer."),
				ExtraFields: map[string]interface{}{
					ThinkingExtraFieldGeneratorKey: ThinkingGeneratorGemini,
				},
			}, CandidatesIndex: 0},
			{Block: TextBlock("Final answer."), CandidatesIndex: 0},
		},
	}
	adapter := StreamingAdapter{S: mockGen}
	...
	if len(candidate.Blocks) != 2 {
		t.Fatalf("blocks = %d, want thinking and content blocks", len(candidate.Blocks))
	}
	block := candidate.Blocks[0]
	if block.BlockType != Thinking {
		t.Fatalf("block type = %s, want %s", block.BlockType, Thinking)
	}
	if got, want := block.Content.String(), "Considering the constraints.\nChecking the final answer."; got != want {
		t.Fatalf("compressed Gemini thinking content = %q, want %q", got, want)
	}
	if got := block.ExtraFields[ThinkingExtraFieldGeneratorKey]; got != ThinkingGeneratorGemini {
		t.Fatalf("thinking generator extra field = %v, want %s", got, ThinkingGeneratorGemini)
	}
}
