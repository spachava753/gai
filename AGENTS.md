...
- Root Go module (go.mod) with all library code colocated for ease of import
  ...
  - Generated OpenAPI clients: `internal/openai/` uses oapi-codegen; `internal/cerebras/`, `internal/deepseek/`, `internal/opencode/`, `internal/openrouter/`, and `internal/zai/` still use OGEN (the Cerebras/DeepSeek/Z.AI clients are no longer used by their public adapters).
